...
class OfficeAdminRequiredMixin(LoginRequiredMixin):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if str(request.user.u_role).lower() != 'office_admin':
...
class OfficeAdminProfileView(OfficeAdminRequiredMixin, TemplateView):
    def get(self, request):
        if not hasattr(request.user, "user_id"):
            return redirect("login")
        ...
        return render(request, "office_admin/profile.html", context)
...
class EmployeeManagementView(OfficeAdminRequiredMixin, TemplateView):
    def get(self, request):
        if not hasattr(request.user, "user_id"):
            return redirect("login")
        ...
        return render(request, "office_admin/employee_management.html", data)
...
class EmployeeSearchView(OfficeAdminRequiredMixin, View):
    def get(self, request):
        ...
        with connection.cursor() as cursor:
            sql = """
                ...
                LEFT JOIN departments d ON e.department = d.department_id
                LEFT JOIN employee_contact_details cd ON e.employee_id = cd.employee_id
                WHERE e.is_deleted = 0 AND u.u_role != 'OFFICE_ADMIN' AND u.u_role != 'HR_ADMIN' AND u.user_id != %s
            """
...
class EmployeeExportView(OfficeAdminRequiredMixin, View):
    def get(self, request):
        search_query = request.GET.get("q", "")
        department_id = request.GET.get("department_id")
...
class EmployeeDeleteView(OfficeAdminRequiredMixin, View):
    def post(self, request, employee_id):
        try:
            ...
            response = redirect("office_employee_management")
...
class AddEmployeeView(OfficeAdminRequiredMixin, TemplateView):
    ...
    def get(self, request):
        ...
        response = render(request, "office_admin/add_employee.html", data)
...
class LeaveActionView(OfficeAdminRequiredMixin, View):
    def post(self, request):
...
class LeaveApprovalView(OfficeAdminRequiredMixin, TemplateView):
    def get(self, request):
        if not hasattr(request.user, "user_id"):
            return redirect("login")
        ...
        return render(request, "office_admin/leave_approval.html", data)
    ...
    def get_leave_list(self, request, user_id):
        ...
        with connection.cursor() as cursor:
            base_sql = f"""
                ...
                WHERE (SELECT u_role FROM users WHERE user_id = {user_id}) = 'OFFICE_ADMIN'
                    AND u.u_role IN ('EMPLOYEE', 'HYBRID')
                    AND (
                        (l.L_status = 'PENDING')
                        OR
...
class LeaveExportView(OfficeAdminRequiredMixin, View):
    def get(self, request):
        export_format = request.GET.get("format", "excel")
        ...
        with connection.cursor() as cursor:
            cursor.execute(f"""
                ...
                JOIN employees e ON l.employee_id = e.employee_id
                JOIN users u ON e.user_id = u.user_id
                WHERE u.u_role IN ('EMPLOYEE', 'HYBRID')
...
class LeaveManagementView(OfficeAdminRequiredMixin, TemplateView):
    def get(self, request):
        ...
        return render(request, "office_admin/leave_management.html", data)
...
class DownloadLeavePDFView(OfficeAdminRequiredMixin, View):
...
class CancelLeaveRequestView(OfficeAdminRequiredMixin, View):
    def post(self, request):
        leave_id = request.POST.get("leave_id")
        user_id = request.user.user_id
        ...
        try:
            with connection.cursor() as cursor:
                ...
                if not record:
                    response = redirect("office_leave_management")
...
class RequestLeaveView(OfficeAdminRequiredMixin, TemplateView):
    def get(self, request):
        ...
        return render(request, "office_admin/request_leave.html", data)
...
class PayrollView(OfficeAdminRequiredMixin, TemplateView):
    template_name = "office_admin/payroll.html"
...
class GeneratePayslipsView(OfficeAdminRequiredMixin, View):
    def post(self, request):
        try:
...
class ExportPayslipsView(OfficeAdminRequiredMixin, View):
    def get(self, request):
        export_format = request.GET.get("format", "excel")
...
class MyPayrollView(OfficeAdminRequiredMixin, TemplateView):
    template_name = 'office_admin/mypayroll.html'
...
class DownloadPayslipPDFView(OfficeAdminRequiredMixin, TemplateView):
    def get(self, request):
        user_id = request.user.user_id
...
class AccessDeniedView(View):
    ...
    def get(self, request):
        return render(request, "office_admin/403.html", status=403)
